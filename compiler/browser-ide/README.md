...
- `Run` compiles all open Scala files, links, and executes either `object Main` / `Main.main(args: Array[String])`, or a single top-level `@main`.
- Use `New File` to add more Scala files. Inline quoted macros defined in any open source file are available to the other open files.
...
- Choose the `Author Macro` example to compile a macro source file and a program using it in the same browser session.
- Use `Import Macro` to add a macro API jar and a Scala.js implementation IR zip to the current browser session.
