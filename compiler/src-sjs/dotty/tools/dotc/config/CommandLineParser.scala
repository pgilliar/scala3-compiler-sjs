...
object CommandLineParser:
  ...
  def tokenize(line: String, errorFn: String => Unit): List[String] =
    ...
    // Skip to the next whitespace word boundary; record embedded quotes.
  ...
  /** Expands arguments starting with `@`.
   */
  def expandArg(arg: String): List[String] =
    val file = dotty.tools.io.File(arg.stripPrefix("@"))(using scala.io.Codec.UTF8)
    if !file.exists then
      System.err.println(s"Argument file ${file.name} could not be found")
      Nil
    else
      def stripComment(s: String) = s.indexOf('#') match
        case -1 => s
        case i => s.substring(0, i)
      val params = file.lines().map(stripComment).filter(!_.isEmpty).mkString(" ")
      tokenize(params)
