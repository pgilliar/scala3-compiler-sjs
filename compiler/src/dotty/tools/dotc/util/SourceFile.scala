...
object SourceFile {
  ...
  def relativePath(source: SourceFile, reference: String): String = {
    ...
    if jpath eq null then
      ...
      platformDependent {
        val sourcePath = java.nio.file.Paths.get(file.path).toAbsolutePath.normalize
        val refPath = java.nio.file.Paths.get(reference).toAbsolutePath.normalize

        if sourcePath.startsWith(refPath) then
          // We consistently use forward slashes as path element separators
          // for relative paths across platforms.
