...
import dotty.tools.dotc.interfacessjs
...
class CodeGen(val backendUtils: BackendUtils, val primitives: DottyPrimitives, val frontendAccess: PostProcessorFrontendAccess, val ts: CoreBTypesFromSymbols)(using Context) {
   ...
   *  `dotty.tools.dotc.interfacessjs.AbstractFile`.
   */
  private def convertAbstractFile(absfile: dotty.tools.io.AbstractFile): interfacessjs.AbstractFile =
    new interfacessjs.AbstractFile {
      override def name: String = absfile.name
      override def path: String = absfile.path
      override def diskPath: Option[String] = Option(absfile.file).map(_.getPath)
    }
