...
import dotty.tools.dotc.interfacessjs.Diagnostic.{ERROR, INFO, WARNING}
...
class Diagnostic(
  ...
  val level: Int
) extends interfacessjs.Diagnostic:
  ...
  override def position: Option[interfacessjs.SourcePosition] =
    if (pos.exists && pos.source.exists) Option(pos) else None
  override def message: String =
    msg.message.replaceAll("\u001B\\[[;\\d]*m", "")
  override def diagnosticRelatedInformation: JList[interfacessjs.DiagnosticRelatedInformation] =
