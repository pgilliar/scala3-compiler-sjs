...
import dotty.tools.dotc.interfacessjs.SourceFile
...
enum MessageFilter:
  def matches(message: Diagnostic): Boolean =
    ...
    case SourcePattern(pattern) =>
      val source = message.position.getOrElse(NoSourcePosition).source
      val path = source.diskPath
        .getOrElse(source.path)
