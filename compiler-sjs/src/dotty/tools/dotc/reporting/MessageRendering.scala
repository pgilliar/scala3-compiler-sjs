...
trait MessageRendering {
  ...
  private def hl(str: String)(using Context, Level): String =
    summon[Level].value match
      case interfacessjs.Diagnostic.ERROR   => Red(str).show
      case interfacessjs.Diagnostic.WARNING => Yellow(str).show
      case interfacessjs.Diagnostic.INFO    => Blue(str).show
  ...
  private def diagnosticLevel(dia: Diagnostic): String =
    dia match {
      ...
      case _ => dia.level match // Diagnostic isn't sealed (e.g. created in the REPL) so provide a fallback
        case interfacessjs.Diagnostic.ERROR   => "Error"
        case interfacessjs.Diagnostic.WARNING => "Warning"
        case interfacessjs.Diagnostic.INFO    => "Info"
    }
